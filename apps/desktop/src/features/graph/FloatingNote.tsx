 ...
 * ## 它解决什么
 *
 * 停靠式预览（`GraphPreview`）一次只能显示一篇，而且钉在画布右侧。想"一边看着 A、
 * 一边顺着 A 里的链接读 B"就只能来回点，看完 B 还得自己找回 A。浮动面板允许**同时开好几个**
 * 这样的阅读窗口：各自可拖动、可缩放，点一下置顶（`onRaise`），`Esc` 关掉最上面那个。
 *
 * ## 它是**完全受控**的
 *
 ...
 *    它现在的排除名单里只有 `.mn-graph-preview`。面板自己会 `stopPropagation` 兜住这一条
 ...
 * 所以这里复用的是与 `GraphPreview` **同一条**渲染链路，逐字对齐：
