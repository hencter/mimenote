...
mod tests {
    ...
    /// 临时探针：比较 sqlite 的查询计划与 `optimize` 的影响（跑完就删）。
    #[test]
    #[ignore]
    fn probe_query_plans() {
        let index = SearchIndex::open_in_memory().unwrap();
        index.begin_rebuild().unwrap();
        for i in 0..3000 {
            index
        ...
        let time = |label: &str, sql: &str, query: &str| {
            let started = std::time::Instant::now();
            let mut statement = index.conn.prepare(sql).unwrap();
            let rows = statement
                ...
                .unwrap()
                .count();
            eprintln!("{label}: {} ms（{rows} 行）", started.elapsed().as_millis());
        };

        let plan = |label: &str, sql: &str, query: &str| {
            let mut statement = index
                .conn
                .prepare(&format!("EXPLAIN QUERY PLAN {sql}"))
                .unwrap();
            let rows: Vec<String> = statement
                .query_map(params![query], |row| row.get::<_, String>(3))
                .unwrap()
                .map(|row| row.unwrap_or_default())
                .collect();
            eprintln!("{label} 计划：{rows:?}");
        };

        for query in ["关键词", "全文搜索"] {
            let pattern = format!("\"{}\"*", space_cjk(query));
            eprintln!("--- {query}（匹配 {} 行）---", count(&pattern));
            let current = QUERY_MATCH.replace("LIMIT ?2", "LIMIT 50");
            plan("现状", &current, &pattern);
            time("现状", &current, &pattern);
            time("CROSS JOIN", CROSS_JOIN_TRY, &pattern);
            time("CTE+LIMIT", CTE_TRY, &pattern);
        }

        let started = std::time::Instant::now();
        index
            .conn
            .execute_batch("INSERT INTO lines_fts(lines_fts) VALUES('optimize');")
            .unwrap();
        eprintln!("optimize：{} ms", started.elapsed().as_millis());
        for query in ["关键词", "全文搜索"] {
            let pattern = format!("\"{}\"*", space_cjk(query));
            let current = QUERY_MATCH.replace("LIMIT ?2", "LIMIT 50");
            time("optimize 后 现状", &current, &pattern);
            time("optimize 后 CROSS JOIN", CROSS_JOIN_TRY, &pattern);
            time("optimize 后 CTE+LIMIT", CTE_TRY, &pattern);
        }
    }

    const CROSS_JOIN_TRY: &str = "
SELECT l.rel_path, l.line, -bm25(lines_fts) AS score
FROM lines_fts CROSS JOIN lines l ON l.id = lines_fts.rowid
WHERE lines_fts MATCH ?1
ORDER BY score DESC, l.rel_path ASC, l.line ASC
LIMIT 50";
    const CTE_TRY: &str = "
WITH matched AS (
  SELECT rowid AS id, -bm25(lines_fts) AS score
  FROM lines_fts WHERE lines_fts MATCH ?1
  ORDER BY score DESC, id ASC LIMIT 50
)
SELECT l.rel_path, l.line, matched.score
FROM matched JOIN lines l ON l.id = matched.id
ORDER BY matched.score DESC, l.rel_path ASC, l.line ASC";
}
