...
全文搜索实测（`cargo test -p mn-index --release -- --ignored --nocapture`，合成 Vault：1 万笔记、每篇 29 行、共 **29.1 万行**，FTS 库 113.8 MB）：

| 项 | 实测 |
| --- | --- |
| 索引构建（与链接/标签同一次遍历，后台、可取消） | ≈ 5.9 s（链接+标签）/ ≈ 12 s（含 FTS5） |
| 查询：命中 1000 行 | **49–75 ms** |
| 查询：命中 29 万行（查询词在每一行里都出现） | **0.8–1.1 s** |
| 单独的全量计数 `COUNT(*)` | 12 ms（**不是**瓶颈） |

慢的那一档是**排序的固有成本**：`bm25()` 要给全部命中打分，`ORDER BY score LIMIT 50` 也必须先算完
（已实测 `CROSS JOIN` / `CTE+LIMIT` / 去掉 `ORDER BY` / 加大页缓存 / mmap 等变体，都在 42–57 ms 同一量级，
说明没有便宜的优化空间）。真实笔记库里"某个词出现在每一行"极少见，常见词查询落在几十毫秒档；
面板对请求做了 150ms 防抖 + 竞态丢弃，不会因为连续输入堆积查询。

索引构建在**每次打开 Vault** 时会随链接/标签索引一起重跑（可取消、不阻塞 UI）；跨会话复用缓存、
按文件 mtime 增量更新属于 M5「增量索引」的范围，`<Vault>/.mimenote/cache/search.db` 删掉即可重建。
