...
| 本地图片 | 预览与编辑器都渲染本地图片（`![](附件/图.png)`、`![[附件/图.png]]`；相对路径、`../`、根绝对三种写法）。图片经 **asset 协议逐文件授权**：路径先由 `path_guard` 逐级检查符号链接与越界，只放行通过校验的那一个文件（见 ADR-0007）；**点击放大**（灯箱：缩放/平移/`Esc` 关闭），加载失败**回退成占位元素**而不是裂图 |
| 知识图谱 | `Ctrl+G` 打开**卡片画布**：每篇笔记是一张卡片（标题/路径/标签/出入度），**文件夹自动成组**并可点开收起，连线**入链虚线、出链实线**（不用按 Ctrl 就能看清关联），指向还不存在的笔记时直接标出**用户写下的目标名**；单击卡片就地预览正文、双击打开进编辑器；画布可平移缩放、卡片可拖动，位置可持久化（见 ADR-0010） |
| 设置与菜单 | 标题栏**菜单**按分类列出全部命令（带快捷键，`↑↓`/`Enter`/`Esc` 可用）；**设置页**（`Ctrl+,`）四个分区：外观（主题/界面字号/编辑器字号）、编辑器（自动保存延迟/Tab 宽度）、Vault（CSS 片段开关、索引状态与重扫）、关于（版本与 Vault 统计）；所有设置即时生效并持久化 |
...
| `cargo test -p mn-index` | 81 个单元测试通过（链接解析/歧义消解/增量更新/构建取消、重命名与链接精确改写、标签索引、FTS5 搜索与恶意查询、图谱组装与截断）（另有 2 个基准默认忽略） |
| `cargo test -p mimenote` | 52 个宿主单元测试通过（IPC 错误映射、路径解析、建笔记、重命名、标签、搜索、图片授权、图谱、片段、写锁、启动参数） |
| `pnpm test` | 23 个测试文件 / 392 个测试通过 |
| `pnpm test:e2e:ui` | 19 个用例通过（系统 Edge，真实 Chromium，约 7 秒） |
| `pnpm test:e2e:app` | 13 个用例通过（真实 release 二进制 + 真实磁盘，约 33 秒） |
...
16. **所见即所得验证**：打开 `项目/设计文档.md`，编辑器里应直接看到渲染后的标题/表格/代码块，而**不是** `#` 与反引号；把光标移进某段粗体（`**…**`）应立刻露出星号，移开又隐藏；任务列表 `- [ ]` 可以直接点
17. **知识图谱验证**：`Ctrl+G` → 每篇笔记是一张卡片、文件夹自动成组；点卡片右侧出现正文预览；`路线图.md` 的卡片被选中时，**指向它的线是虚线、它指出去的线是实线**；点文件夹标题可收起/展开；拖动空白处平移、`Ctrl+滚轮` 缩放、`0` 适应窗口；双击卡片回到编辑器
18. **设置与菜单验证**：标题栏左侧菜单按钮 → 按分类列出全部命令（带快捷键）；`Ctrl+,` 打开设置 → 改编辑器字号应立即生效、改自动保存延迟后输入不再频繁写盘；「Vault」分区能看到索引进度与重扫按钮
...
3. 标签面板只做**展示与跳转**，改标签目前要手动编辑 frontmatter 或正文；搜索**回车不跳到命中行**（编辑器还没有 `openAt(relPath, line)` 入口）
4. **图谱不随编辑自动刷新**（切 Vault、点"重新读取图谱"、索引就绪时才会重拉；`load()` 会重置视口，做"存盘后刷新"需要一个保留视角的变体）；手工拖动会覆盖自动布局，「重新自动排布」一键复位；折叠状态不持久化（每次默认全展开）
5. 删除走 Vault 内回收站，未对接系统回收站；恢复界面在 M2
6. 冲突检测基于 mtime（同毫秒内两次独立修改理论上有漏检窗口）
7. 大文档（>5MB）预览仍在主线程渲染；编辑器装饰按视口计算，但**光标移动也会重算**（"进入即露原文"的必然代价）
8. E2E 未覆盖：多窗口、插件（M4）、以及"标签/搜索/图谱在超大 Vault 下的表现"（功能已覆盖，规模未覆盖）
...
| 图谱数据（`graph_data`） | 一次 IPC 可接受 | 1 万笔记 / 2 万条链接：组装 **55–85 ms**、报文 1.0 MB（按度数截断到 3000 节点）；含 5% 悬空链接时 163–364 ms（悬空解析是 `resolve_target` 的全库后缀兜底，见 `mn-index` 注释） |
