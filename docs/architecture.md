...
| [ADR-0026](adr/0026-dockable-modules.md) | **视图模块的三个停靠区**（左/右/底 + 区内顺序）：`dock-layout.ts` 是纯函数并带"每个模块恰好一次"的不变式；只存**位置**不存可见性（四条切换快捷键一字不改）；拖拽（空区在拖动期间出现"可放"轨道）与键盘等价物（`Alt+1/2/3`、同区 `Alt+方向键`）；**标签栏搬到窗口顶部**（`.mn-app` 的直接子节点，删掉 `.mn-main:has(> .mn-tabs)`）；**通用右键菜单**接入文件树行 / 标签页 / 停靠模块头 / 图谱卡片（菜单项直接执行既有命令） | 已采纳 |
...
| [ADR-0029](adr/0029-titlebar-three-regions.md) | **标题栏定型为左/中/右三区**（网格 `1fr / 2fr / 1fr`：左 = 哪个库、中 = 哪一篇、右 = 多大 + 出口动作），**当前笔记路径从编辑器面板搬进中区** —— 顶部高度因此恒定 34px，路径在阅读/图谱视图里也看得见；三区只是 `div`，标题栏整条仍是拖动区（ADR-0017 不变） | 已采纳 |
| [ADR-0035](adr/0035-container-split-tree.md) | **容器切割树**（布局模型层）：模块与笔记统一成"**标签**"，界面 = 一棵二叉切割树（`Split{axis, ratio, a, b}` / `Leaf{items, active}`）；六条不变式（**每个标签全树恰好一次** —— 拖过去就是移过去、空叶塌缩但树至少留一个叶子、比例夹 [0.15,0.85]、认不出的标签丢弃、整棵不合法退回默认、`active` 必须属于本叶）；操作全是纯函数（`moveItem` 中心/边缘、`removeItem`、`setActive`、`setRatio`、`attachItem`、`normalizeLayout`）；`fromDockLayout` 给出 ADR-0026 的迁移。**UI 接线是后续交付** |
| [ADR-0034](adr/0034-merged-top-row.md) | **顶行合并**：标题栏、文件标签与窗口按钮同一行（三区网格不变，**中区从"当前路径"换成"文件标签栏"**；行高 34→36；这一行永远在，所以窗口按钮不会因"没有标签"而消失）；"我在看什么"挪到**状态栏最左**；标签条声明 `data-tauri-drag-region="false"`（否则按住 `role="tab"` 的 `div` 会被当成拖窗口）；布局契约式改成 `主体 = 窗口 − 标题栏 − 状态栏`（标签栏在标题栏里，不能再减一次） |
...
| **力导向浮动态**（ADR-0023） | 打开即落定、之后 20fps 慢漂移 | `createForceSimulation` 的 `settle()` 在 ego 一跳图上实测 **246 步**（默认参数 `alphaDecay 0.005` = 200 步预算，一次跑完就停）；单步是 `O(n²)` 斥力（宿主上限 300 节点 ⇒ 4.5 万对/步）。漂浮按 **20fps** 推进（`FLOAT_FPS`）：位置一变，卡片（canvas）与连线（SVG/React）都要重画，20fps 下"缓慢漂移"已经是连续的，代价只有 60fps 的三分之一 —— 要 60fps 得先把连线也搬到 canvas（ADR-0023 决策 5） |
...
29. **图谱的力导向与"从 wiki link 引出的连线"的边界**（ADR-0023）：① 力场会把同心环**收紧**（默认档实测一跳 562 → 377、二跳 1124 → 672，"谁在内谁在外"保住、"半径 = 布局算出的数"不再成立；想保住半径用「舒展」档）；② 卡片内那段虚线引线**只在焦点视图**里有 —— 全库视图的卡片是紧凑卡片（没有正文位置），那里仍然从卡片边界出发；③ 匹配不到文字时降级成"从卡片边界出发"（`fromLink: false`，tooltip 说明原因），相邻同 href 的 `[[甲|A]][[甲|B]]` 会被合并成一个 run 而只能给一个锚点，自环的引线退化成一个点；④ **浮动面板不持久化**（临时阅读姿势），`Esc` 的语义是"先关最上面那个浮窗"；⑤ 漂浮是 **20fps 的慢漂移**（位置一变，卡片 canvas 与连线 SVG 都要重画，后者是 React 渲染）—— 要 60fps 得先把连线也搬到 canvas。
...
33. **停靠布局只解决"位置"**（ADR-0026）：① 同侧多块模块**平分**该区的高度/宽度（`flex: 1 1 0` + `min-height: 96px`），没有每块各自的尺寸记忆；② 不做"同一块面板开两份"（模型的不变式是每个模块恰好出现一次，重复配置整份退回默认）；③ 底部区高度是全局一份；④ 可见性仍归各面板自己的开关（`Ctrl+B` / `Ctrl+Shift+L` / `Ctrl+Shift+T` / `Ctrl+Shift+O`），所以"隐藏"与"搬家"是两件互不覆盖的事；⑤ 空区只在**拖动进行中**渲染成一条可放的轨道。

35. **连线走线与语义色相的边界**（ADR-0028）：① 弧按**当前极角**实时算（漂浮时线会跟着微调，这是刻意的：形状必须跟着卡片走）；② 同环的弧**不随张力变直**（结构性走线），跨环的径向切线才吃张力滑杆；③ 跨环是"朝外鼓"的近似，不做真正的避障；④ 同一对笔记之间的多条链接仍然只画一条（`count > 1` 在 tooltip 里）；⑤ 连线仍是 React 渲染的 SVG，漂浮仍 20fps —— 把连线搬进 canvas 是 taper / 发光 / 流动这些效果的前置，没有做；⑥ 色相令牌 `--mn-edge-out` / `--mn-edge-in` 是**可选**的（缺省落到 `--mn-warning` / `--mn-link`），**没有**加进 `REQUIRED_TOKENS` —— 那会让所有用户自定义主题在启动时报缺令牌。
...
36. **标题栏三区的边界**（ADR-0029）：① 中区（当前笔记路径）**上限是窗口的一半** —— 长路径走省略号（完整路径在 `title` 提示里），窗口窄到 1000px 上下时右区的统计也开始省略（状态栏里那份是完整的）；② 标题栏带 `user-select: none`（它同时是拖动区）⇒ **路径不可选中复制**，要"复制路径"应该走文件树/标签的右键菜单，而**本轮没有做那个菜单项**；③ 中区表达的是"**最后打开的那一篇**"（与窗口标题同一口径），全库图谱视图下它与画布中心不是一回事 —— 只有关系图的圆心才等于当前笔记；④ 三区内容**不可配置**：ADR-0026 的停靠模型只管视图模块，标题栏不在它的作用域内，想往里加东西仍然是改 `App.tsx`。
...
42. **容器切割树的边界**（ADR-0035，模型层已交付、**UI 未接**）：① 拖拽落点提示、每叶一条标签栏、分隔条、键盘等价物都还是后续交付 —— 界面目前仍是"全窗口唯一一条标签栏在标题栏中区"（ADR-0034）+ ADR-0026 的三区停靠；② 迁移期两套落盘格式并存（`mimenote.ui.v1` 的 `dockLayout` 与 `mimenote.tabs.v1`），接线时要"读旧 → 转树 → 只写树"且旧键保留一轮；③ 模型不判"标签还存不存在"（删掉的笔记 / 关掉的模块由上层对账，`isKnownItem` 钩子）；④ 比例夹紧意味着"拖到 0"实际落在 0.15，拖回来时起点不是 0；⑤ **同一篇笔记不能左右对照**（不变式 1 的直接后果，要支持得先做多文档模型）；⑥ `note:` 前缀占用标签命名空间，加视图模块时要看一眼。

41. **顶行合并的边界**（ADR-0034）：① 品牌/库名与统计/导出被压进同一条 36px 的带子，窄窗口下先走省略号；标签条只拿窗口中间那一半（1fr/2fr/1fr 网格），标签多时横向滚动；② 路径的可发现性下降 —— 它从窗口顶部正中挪到状态栏左侧，"完整相对路径"要低头看；③ 应用层 E2E 需要 `tauri build --no-bundle` 重建后重跑（它跑的是打包进二进制的旧前端）；④ 分屏/容器切割（每叶一条标签栏）**还没做** —— 本 ADR 只是把"全窗口唯一那条标签栏"安顿到标题栏中区，ADR-0026 的三区停靠仍然管视图模块。
