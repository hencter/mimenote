...
3. `src-tauri` 只做三件事：持有会话状态、把 mn-core/mn-index 能力暴露成 IPC 命令、把错误映射成稳定错误码。**不放业务逻辑**。IPC 命令按**领域**组织（`commands/` 一个领域一个文件：vault / notes / tags / trash / links / graph / search / system，资源/附件/导出早已独立为 assets / attachments / export / site_export），每个领域的命令集合在 `lib.rs` 的 `generate_handler!` 里按同一分块注册 —— 这就是 M4 插件系统 Plugin Manifest / Permission Model 的 capability boundary（`commands/mod.rs` 的模块文档里有领域 → 命令对照表）。
...
| 界面 | 投影自 | 为什么 |
| --- | --- | --- |
| 应用菜单（文件树格子右下角） | **命令注册表**（`app/builtin-commands.ts` 的 `BUILTIN_COMMANDS`，按 `category` 分组） | 菜单与命令面板读同一份数据。若菜单自己维护一张表，"面板里能搜到、菜单里没有"这种漂移迟早出现；菜单只**跳过**面板自己托管的那几条命令（避免同一个动作出现两行） |
...
**没有独立标题栏，窗口控件住进内容顶行**（ADR-0017 → ADR-0038）：宿主的 `tauri.conf.json`
关掉了系统装饰（`decorations: false`），而从 ADR-0038 起连自绘的 `.mn-titlebar` 也退役了 ——
内容（切割树）从窗口最顶边开始，`主体 = 窗口 − 状态栏`。配套的三件事必须记住：

① **拖动区由标签条承担**（`data-tauri-drag-region="deep"`，Tauri 注入脚本负责拖动与
**双击最大化**，并沿 composed path 自动跳过按钮、`role=tab`、`contenteditable` 这类可点击元素，
因此标签点击与编辑器选区都不受影响，也不需要 `stopPropagation`）；
② 关窗口只能靠我们自己的按钮（`features/window/`），能力集里那 6 条 `core:window:allow-*`
就是为此而开；三个按钮住在**主叶标签条右端**（滚动区之外的固定槽，标签再多也挤不掉），
所以主叶标签条即使没有标签也渲染；
③ 最大化图标**从真实窗口读回来**（`isMaximized` + `onResized` 订阅），不自己做本地开关
—— 双击标签条、`Win+↑`、拖到屏幕上沿都会改变窗口状态，本地开关必然与之脱节。

业务信息各自归位：库名/统计在**状态栏**（统计那一份是唯一一份，含「已截断」标记）、
`导出`在**文件树工具栏**（与新建/重命名/移动同属内容层的工具区域；文件树收起时走命令面板）、
"我在看什么"在状态栏最左（`data-main-path`，ADR-0034 起）。
...
**契约的自动一致性校验**（`pnpm check:ipc`，CI 门禁之一，脚本在 `scripts/check-ipc.mjs`）：

- **命令名三方一致**：Rust `tauri::generate_handler![]` ↔ `src/ipc/client.ts` 的 `call<…>('…')` ↔ `src/ipc/mock-adapter.ts` 的 `case '…'`；
- **ErrorCode 集合一致**：`mn_core::ErrorCode::as_str()` + 宿主 `error.rs` 新增码（`UNSUPPORTED_MEDIA` / `INDEX_NOT_READY` / `INTERNAL`）↔ TS 的 `ErrorCode` 联合与 `KNOWN_CODES` 白名单（`CANCELLED`/`UNKNOWN` 是前端专有码，后者不进白名单）；
- **DTO 字段逐字段一致**：脚本 `DTO_MANIFEST` 登记的每一组 TS interface ↔ Rust struct（`interface extends` 会先展开）；
- **字符串枚举一致**：`LinkKind` / `TagSource` / `IndexPhase` / `TagSkipReason` 四组字面量联合 ↔ Rust 单元枚举（按各自的 `rename_all` 换算）。

新增命令 / DTO 的流程：① Rust 侧写命令与 DTO（`serde(rename_all = "camelCase")`）并注册进 `generate_handler!`；② `types.ts` 镜像 DTO、`client.ts` 加调用方法；③ `mock-adapter.ts` 实现同名 `case`；④ 新 DTO 登记进 `scripts/check-ipc.mjs` 的 `DTO_MANIFEST`（新错误码 / 新枚举同理）；⑤ 本地跑 `pnpm check:ipc` 确认四层全绿。漏掉任何一侧，CI 会报出"哪一侧多、哪一侧少"。
...
| [ADR-0029](adr/0029-titlebar-three-regions.md) | **标题栏定型为左/中/右三区**（网格 `1fr / 2fr / 1fr`：左 = 哪个库、中 = 哪一篇、右 = 多大 + 出口动作），**当前笔记路径从编辑器面板搬进中区** —— 顶部高度因此恒定 34px，路径在阅读/图谱视图里也看得见；三区只是 `div`，标题栏整条仍是拖动区（ADR-0017 不变）。**中区的内容后又经 ADR-0034（文件标签）与 ADR-0035（纯拖动区）两次替换，"我在看什么"现在在状态栏最左；ADR-0038 把整个标题栏退役** | 已采纳（后被 0038 修订） |
| [ADR-0038](adr/0038-titlebar-retired.md) | **独立标题栏退役**（Issue #8）：`.mn-titlebar` 与三区整组删除，内容从窗口最顶边开始（`主体 = 窗口 − 状态栏`）；业务控件下移 —— 统计/库名 → **状态栏**、`导出` → **文件树工具栏**；三个窗口按钮住进**主叶标签条右端**的固定槽（滚动区之外），主叶标签条因此**空态也渲染**；拖动区改由标签条承担（`deep`，Tauri 自动跳过 `role=tab`/按钮）；主叶判据 = 当前文档格 → 内容叶（`contentLeafId`）→ DFS 第一格 | 已采纳 |
| [ADR-0039](adr/0039-graph-visual-refresh.md) | **图谱视觉刷新（聚光灯）**：画布加**世界坐标点阵**（跟平移/缩放走，太密则跳过）与中心极淡聚光；卡片圆角 6→10、**当前笔记常驻淡光晕、悬停/选中更亮**（描边模拟，不引入 canvas 阴影）；HUD 改**玻璃浮层 + 分段控件 + 无边框胶囊**（DOM 与 `data-*` 抓手不变）；状态栏把**图谱按钮从编辑/阅读组里分出**（细竖线分隔） | 已采纳 |
...
| [ADR-0034](adr/0034-merged-top-row.md) | **顶行合并**：标题栏、文件标签与窗口按钮同一行（三区网格不变，**中区从"当前路径"换成"文件标签栏"**；行高 34→36；这一行永远在，所以窗口按钮不会因"没有标签"而消失）；"我在看什么"挪到**状态栏最左**；标签条声明 `data-tauri-drag-region="false"`（否则按住 `role="tab"` 的 `div` 会被当成拖窗口）；布局契约式改成 `主体 = 窗口 − 标题栏 − 状态栏`。**后随 ADR-0035（标签进各自格子、中区回归纯拖动区）与 ADR-0038（独立标题栏退役、窗口按钮进主叶标签条）** | 已采纳（后被 0038 修订） |
...
36. **顶部空间的边界**（ADR-0029 → 0034 → 0035 → **0038 标题栏退役**）：① 顶部不再有独立标题栏，内容从窗口最顶边开始；拖动区是**每一条标签条**（`data-tauri-drag-region="deep"`），标签与按钮的点击由 Tauri 的脚本自动跳过，不需要手写 `false`；② 窗口按钮（最小化/最大化/关闭）住在**主叶标签条右端**的固定槽（滚动区之外），主叶标签条因此**空态也渲染** —— 主叶判据 = 当前文档格 → 内容叶（`contentLeafId`）→ DFS 第一格，不能退回 `leaves()[0]`（那通常是文件树格）；③ "我在看什么"（当前笔记/附件路径）在**状态栏最左**（ADR-0034），状态栏路径表达的是"**最后打开的那一篇**"（与窗口标题同一口径），全库图谱视图下它与画布中心不是一回事 —— 只有关系图的圆心才等于当前笔记；④ 顶部的内容不可配置，想往里加东西仍然是改 `LeafTabs` / `TreeHost`；⑤ 已知代价：主叶被拖到窗口下半部时窗口按钮跟着走、主叶窄于 126px 时按钮会被格子裁掉、标签铺满时拖动面积只剩标签间隙（ADR-0038 记了全表）。
